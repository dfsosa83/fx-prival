...
- **H1**: each bar = 1 hour, predict move over next **10 bars (10 hours)**.
- **D1**: each bar = 1 day, predict move over next **5 bars (~1 trading week)**.
...
- **BUY** when $r_{t \to t+h} \ge \theta^{+}$
- **SELL** when $r_{t \to t+h} \le \theta^{-}$
- **NEUTRAL** when $\theta^{-} < r_{t \to t+h} < \theta^{+}$
...
2. **Threshold tuning** — θ⁺ / θ⁻ per pair via walk-forward, not in-sample; too tight → all NEUTRAL, too loose → noisy signals.
