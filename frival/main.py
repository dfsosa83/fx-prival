...
def execute_pending():
    ...
    Shadow hygiene (2026-09-18): signals for shadow pairs (e.g.
    EURUSD_AGNOSTIC) are logged to the shared signal JSONL with a `trade`
    payload but MUST never reach the OrderBot — it has no symbol config for
    them and prints "Symbol not found" every hour. Filter them out here so
    the execution layer only ever sees real, live, executables signals.
    """
    ...
    #── prune shadow-pair records from today's signal JSONLs before the bot runs ──
    signal_dirs = [
        Path(__file__).resolve().parent / "output" / "signals",
    ]
    today_files = []
    for base in signal_dirs:
        month_dir = base / _dt.utcnow().strftime("%Y-%m")
        if month_dir.exists():
            today_files += list(month_dir.glob("*.jsonl"))
    pruned = 0
    for f in today_files:
        ...
        for line in lines:
            ...
            # shadow pairs that must never be executed
            if sym.upper().endswith("_AGNOSTIC") or rec.get("shadow", False):
                pruned += 1
                continue
            kept.append(line)
    ...
    if pruned:
        print(f"[execute_pending] pruned {pruned} shadow signal record(s) "
              f"before OrderBot (symbol endswith _AGNOSTIC or shadow=True)")
